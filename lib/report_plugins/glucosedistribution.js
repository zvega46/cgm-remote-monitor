...
glucosedistribution.html = function html(client) {
    ...
    var ret =
        ...
        translate('can be found here') + '</a> ' + translate('and') + ' <a href="https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5467104/">' + translate('here') +
        '</a>.</div><br/><br/>' +
...
//	console.log('TADC',TDC,'days',days);
    ...
    if (PGS < 15) PGS_description = translate('Excellent');
    if (PGS >= 15 && PGS <= 20) PGS_description = translate('Good');
    if (PGS > 20 && PGS <= 25) PGS_description = translate('Moderate');
    if (PGS > 25) PGS_description = translate('Poor');
    ...
    $('<tr><td class="tdborder">' + TDCHourly + unitString + '</td><td class="tdborder">' + GVI + '</td><td class="tdborder">' + PGS + ' (' + PGS_description + ')</td></tr>').appendTo(stabilitytable);
